// SE Module 1x1 Convolution (Restoration: 4→16 channels)
module Conv2D_specific (
    input  logic clk,
    input  logic rst,
    input  logic load_kernel, // 1: load kernel, 0: process input
    input  logic [15:0] in_data, // input data or kernel data
    output logic [15:0] out_data,
    output logic out_valid
);
    // Parameters for SE module restoration convolution (1x1, 4->16 channels)
    localparam IN_CHANNELS = 4;   // Reduced channels from first SE conv
    localparam OUT_CHANNELS = 16; // Restore to original channel count
    localparam KERNEL_SIZE = 1;   // 1x1 convolution for SE module
    
    // Kernel storage for 1x1 convolution (OUT_CHANNELS x IN_CHANNELS)
    (* ram_style = "distributed" *) logic signed [15:0] kernel [OUT_CHANNELS-1:0][IN_CHANNELS-1:0];
    
    // Input channel buffer (to store all 4 input channels)
    logic [15:0] input_channels [IN_CHANNELS-1:0];
    ...
    always_ff @(posedge clk or posedge rst) begin
        if (rst) begin
            ...
            case (state)
                ...
                LOAD_INPUTS: begin
                    // Load input data for all channels (4 channels from previous conv)
